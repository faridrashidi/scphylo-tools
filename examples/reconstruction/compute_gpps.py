...
# input. GPPS requires ``gurobipy`` with a working Gurobi license, Ruby, and the
# ``gpps_tree`` script. Supply the latter two with ``ruby_executable`` and
# ``tree_script``, ``SCPHYLO_TOOLS_DIR``, or ``PATH``. Regular documentation builds
# render this optional example without executing it; set
# ``SCPHYLO_RUN_EXTERNAL_EXAMPLES=gpps`` in a fully provisioned environment to run
# it (or use ``1`` to run every external example).
df_out = scp.tl.gpps(df_in, alpha=0.0000001, beta=0.1)
df_out.head()
