...
from scphylo.tl.solver.booster._dependencies import prepare_dependencies
from scphylo.tl.solver.booster._reconstruct_big_tree import reconstruct_big_tree
from scphylo.tl.solver.booster._subsamples import subsampling
...
def booster(
    ...
    See Also
    --------
    :func:`scphylo.tl.scite`.
    :func:`scphylo.tl.phiscsb`
    """
    if subsample_dir is not None:
        tmpdir = scp.ul.mkdir(subsample_dir)
    else:
        # tmpdir = scp.ul.tmpdirsys(suffix=".booster")
        # tmpdir = tmpdir.name
        tmpdir = scp.ul.tmpdir(suffix=".booster")
    ...
    if not no_reconstruction:
        scp.io.write(df_input, f"{tmpdir}/_input.SC")
