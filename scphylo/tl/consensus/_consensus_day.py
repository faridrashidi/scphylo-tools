...
def consensus_day(sc1, sc2):
    ...
    if len(common_cells) == 0:
        scp.logg.error("No common cells found in two inputs!")
    sc1_p = sc1.loc[common_cells, :]
    sc2_p = sc2.loc[common_cells, :]

    tree1 = scp.ul.to_tree(sc1_p)
    tree2 = scp.ul.to_tree(sc2_p)
    ...
    scp.logg.debug(artificial_added)
    ...
    # convert back to the scphylo tree format
    ...
    for e, u, _ in final_tree.edges.data("label"):
        ...
        i += 1
    data = scp.ul.to_cfmatrix(final_tree)
    final_tree = scp.ul.to_tree(data)
