...
def booster(
    ...
    """
    dirbase = scp.ul.dirbase(genotype_file)
    # dirname, basename = scp.ul.dir_base(genotype_file)

    scp.settings.verbosity = "info"
    if not no_reconstruction:
        scp.settings.logfile = f"{dirbase}.booster.log"
    ...
    if not no_reconstruction:
        scp.io.write(df_out, f"{dirbase}.booster.CFMatrix")
