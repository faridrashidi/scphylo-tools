...
def bifiltering(df, cellr, mutr, time_limit=3600):
    ...
    scp.logg.info("1. declaring variables", time=True)
    A, C, S = {}, {}, {}
    for i in range(n):
        A[i] = {}
        for j in range(m):
    ...
    scp.logg.info("2. adding constraints", time=True)
    for i in range(n):
        for j in range(m):
    ...
    scp.logg.info("3. adding objective", time=True)
    model.setObjective(
        gp.quicksum(A[i][j] * M[i, j] for i in range(n) for j in range(m)),
        gp.GRB.MAXIMIZE,
    )

    scp.logg.info("4. start solving", time=True)
    model.optimize()
